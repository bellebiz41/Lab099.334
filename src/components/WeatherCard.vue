...
<template>
  <section class="vh-100" >
    <div><h1 style="text-align: center;" class="text_box"><strong>"Weather" {{travelItem.name}} </strong></h1></div>
    <div class="container py-5 h-100">
      <div class="row d-flex justify-content-center align-items-center h-100">
        <div class="col-md-8 col-lg-6 col-xl-4" v-for="(i,index) in items" :key="index">
  
        <div class="card" style="color: #ad61c3; border-radius: 70px;" >
            <div class="card-body p-4" >
              <div class="d-flex">
                <h6 class="flex-grow-1">DATE : {{ i.dt_txt.split(" ")[0] }}</h6>
                <h6>TIME : {{ i.dt_txt.split(" ")[1].slice(0, 5) }} m.</h6>
            </div>
            
            <div class="d-flex flex-column text-center mt-5 mb-4">
              <h6 class="display-4 mb-0 font-weight-bold" style="color: #1C2331;"> {{ (((i.main.temp - 32) / 1.8) / 6).toFixed(2) }}°C </h6>
              <span class="small" style="color: #dc80c5">{{ i.weather[0].main }}</span>
            </div>
            <div class="d-flex align-items-center">
              <div class="flex-grow-1" style="font-size: 1rem;">
                  <div><i class="fas fa-wind fa-fw" style="color: #868B94;"></i> <span class="ms-1"> wind speed : {{ i.wind.speed }} ms/s </span></div>
                  <div><i class="fas fa-tint fa-fw" style="color: #868B94;"></i> <span class="ms-1"> Humidity : {{ i.main.humidity }}% </span></div>
                  <div><i class="fas fa-sun fa-fw" style="color: #868B94;"></i> <span class="ms-1"> Description : {{ i.weather[0].description }} </span></div>
              </div>
...
.card {
  ...
  border: none;
  border-radius: 35px;
  box-shadow: 0px 15px 20px rgba(203, 166, 233, 0.3);
  overflow: hidden;
  position: relative;
  z-index: 1;
}
...
.card:hover {
  box-shadow: 0px 15px 20px rgba(238, 144, 230, 0.6);
}
.card{
  margin-bottom: 50px;
}
